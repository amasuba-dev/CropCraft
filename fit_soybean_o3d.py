...
from morphology.soybean_model import soybean_plant, rotate2d, agg
...
PARAMETER_RANGES = {
    'LEAF_L_SCALE': [0.5, 1.5],
    'LEAF_W_TO_L': [0.8, 0.8],
    'PET_ANG_SCALE': [0.5, 4.0], 
    'NODE_DIST_SCALE': [0.5, 2.0], 
...
TO_FIT = ['LEAF_L_SCALE', 'PET_ANG_SCALE', 'NODE_DIST_SCALE', 'PET_LEN_SCALE', 'NODE_COUNT'] 
...
def soybean_field_parametric(params, n_rows=1, row_len=2., row_dist=0.762, with_dummies=False):
    """Returns soybean field with custom parameterization."""
    xyzs, facs = [], []
    for j in range(n_rows):
        for center_x in np.arange(-row_len/2, row_len/2, 1/params['PLANT_DENSITY']):
            xyz, fac = soybean_plant_parametric(params, with_dummies=with_dummies)
            xyz /= 100.  # cm to m
...
def make_new_mesh(vis, params):
    """Update open3d mesh and visualizer with new soybean field."""
    xyz, fac = soybean_field_parametric(params)
...
def hists_from_maps(maps, masks, bins):
    ...
    for j, m in enumerate(maps):
        ...
        hists[j] = np.histogram(m[masks[j]], bins=bins, density=False)[0] / total_pixels
    return hists


def smooth_sobel(depth, blur_kernel=25, max_depth=1.5):
...
def all_hists_from_depths(depths, d_bins, nx_bins, ny_bins, y_bins, sb_bins, f_y, foreground_thr=1.5, return_maps=False):
    ...
    if return_maps:
        # Return 2D maps instead of histograms
        return depths, sobels, normals, ys, fg_masks
...
def fit_soybean(args, target_depths, w2c_poses, K,
    ...
    def params_from_vector(param_vector):
        """Parameter dictionary from scaled subset vector."""
    ...
    def f(param_vector):
        ...
        if args.results_name == 'debug': 
            ...
            # params = {'LEAF_L_SCALE': 1.5, 'LEAF_W_TO_L': 0.8, 'PET_ANG_SCALE': 1.3355010963990082, 'NODE_DIST_SCALE': 1.0067463452727814, 'PET_LEN_SCALE': 1.0299926952845877, 'AZIMUTH_STD': 60.0, 'NODE_COUNT': 11.125868661166406, 'LEAF_L_STD': 0.0, 'PET1_ANG_STD': 5.0, 'NODE_DIST_STD': 0.0, 'PLANT_DENSITY': 20.0}
            params = {'LEAF_L_SCALE': 0.8715394361093043, 'LEAF_W_TO_L': 0.8, 'PET_ANG_SCALE': 2.823261138302739, 'NODE_DIST_SCALE': 1.315309793544428, 'PET_LEN_SCALE': 0.9292391724572407, 'AZIMUTH_STD': 60.0, 'NODE_COUNT': 3.871583713014778, 'LEAF_L_STD': 0.0, 'PET1_ANG_STD': 5.0, 'NODE_DIST_STD': 0.0, 'PLANT_DENSITY': 20.0}
    ...
    d_bins = np.arange(0.1, view_height + 1e-5, (view_height - 0.1) / args.n_bins_depth)
    ...
    if method == 'tr':
        res = scipy.optimize.minimize(f, [(r[0] + r[1]) / 2 for r in scaled_ranges], 
                                      ...
                                      options={'maxiter': args.bo_max_iter})
...
def preprocess_ns_depth(ns_depth, vfov, scale, mask, uncertain_depth=2, max_depth=2, min_depth=0.1, y_thr=0.5):
...
def get_seg_mask(rgb, seg_thresh=120):
    """Threshold-based segmentation mask from RGB."""
    lab = cv2.cvtColor(rgb, cv2.COLOR_RGB2LAB)
    return lab[:, :, 1] < seg_thresh
...
if __name__ == '__main__':
    ...
    render_dir = os.path.join(work_dir, args.renders_name)
    ...
    for rep in range(args.n_reps):
        ...
        best_params, best_loss, best_loss_history, best_params_history = \
            fit_soybean(args, target_depths, w2c_poses, K, 
                        foreground_thr=view_height, seed=rep)

        print('best params:', best_params)
        print('best_loss:', best_loss)
        print('lai:', leaf_area_index_parametric(best_params, info['row_dist']))
